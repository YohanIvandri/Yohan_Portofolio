...
INNER JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].[dbo].insclaim ic ON ic.nppno = da.nppno
...
FROM #LOOP loo  
INNER JOIN [MACF-DBSTG].[REPL_DBMAF_ECOL].dbo.EC_ARCard a WITH (NOLOCK) ON loo.nppno = a.nppno 
LEFT JOIN [MACF-DBSTG].[REPL_DBMAF_ECOL].dbo.EC_SocialFund sf WITH (NOLOCK) ON a.ARCardId = sf.ARCardId
...
FROM #LOOP loo  
LEFT JOIN [MACF-DBSTG].[REPL_DBMAF_ECOL].dbo.EC_ARCard a WITH (NOLOCK) ON loo.nppno = a.nppno 
...
BEGIN
	 SET @sql = '
                ...
                FROM OPENQUERY([HFMAF], 
                 ''SELECT * FROM EPMAF.DBO.fnEPASUJTPeriod_tyo(''''' + @nppno + ''''','''''+ @approvalDate + ''''')'')
			   '

     SET @sql2 = '
     			SELECT distinct * FROM OPENQUERY([HFMAF],
                 ''SELECT c.nppno, CASE WHEN c.KodeAngsuran = ''''0001'''' THEN (select DendaBungaMenurun from EPMAF.dbo.fnEPGetDendaBungaMenurun(c.nppno, isnull(ic.InsClaimDate,getdate())))
     			          +(1000-((select DendaBungaMenurun from EPMAF.dbo.fnEPGetDendaBungaMenurun(c.nppno, isnull(ic.InsClaimDate,getdate())))%1000)) 
     			           ELSE null END AS OD
                            FROM EPMAF.dbo.CM c 
     			       INNER JOIN EPMAF.dbo.insclaim ic on ic.nppno = c.nppno
     			                   WHERE c.NPPno  = '''''+@nppno+'''''
     			 '') '
...
FROM #temp tm 
INNER JOIN [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.CM c on c.nppno = tm.nppno
INNER JOIN [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.lessee l on l.lesseeno = c.lesseeno
...
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.termination tr on tr.nppno = tm.nppno and tr.statustermination not in('R','B') and tr.terminationcode = 2
LEFT JOIN  [MACF-DBSTG].REPL_DBKONSOL_EPKONSOLCGS.DBO.[CGS.MsStatus] mst on mst.cgsstatusid = cgs.status
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.inscoverdtl icd on icd.nppno = tm.nppno
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.inscoverhdr ich on ich.nosuratcover = icd.nosuratcover
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].dbo.msptinsurance m on m.insid = ich.insid
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].[dbo].insclaim icl ON icl.nppno = tm.nppno
LEFT JOIN  [MACF-DBSTG].[REPL_DBMAF_EPMAF].[dbo].TerminationInsurance ti with(nolock) on ti.ClaimNo=tr.ClaimNo
